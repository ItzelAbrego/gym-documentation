...
- [ ] `POST /centers`: crea centro (nombre, datos de perfil) + obligatoriamente,
      en la misma transacción: la **sucursal principal** del centro (ADR-0010,
      ST-015); el usuario `CENTER_ADMIN` inicial (username = correo validado,
      nombre, password — decisión P-01); el **usuario quiosco** de la sucursal
